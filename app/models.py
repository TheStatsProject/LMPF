...
def create_subscription(username, plan, payment_status):
    ...
    subs_col.insert_one({"username": username, "plan": plan, "payment_status": payment_status})

def record_payment(username, method, details, status):
    from app.database import payments_col
    payments_col.insert_one({"username": username, "method": method, "details": details, "status": status})
